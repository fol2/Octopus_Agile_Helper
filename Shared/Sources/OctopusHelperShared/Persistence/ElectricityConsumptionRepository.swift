...
public final class ElectricityConsumptionRepository: ObservableObject {
    ...
    public func updateConsumptionData() async throws {
        ...
        let lastPageResponse = try await apiClient.fetchConsumptionData(mpan: mpan, serialNumber: serial, apiKey: apiKey, page: totalPages)
        
        guard let serverNewestRecord = firstPageResponse.results.first,
              let serverOldestRecord = lastPageResponse.results.last else {
        ...
        if let localMax = localMaxDate {
            ...
            if needNewerData {
                print("Debug - Server has \(serverNewestRecord.interval_end.timeIntervalSince(localMax) / 3600) hours of newer data")
            }
        } else {
            needNewerData = true
        ...
        if let localMin = localMinDate {
            ...
            if needOlderData {
                print("Debug - Server has \(localMin.timeIntervalSince(serverOldestRecord.interval_start) / 3600) hours of older data")
            }
        } else {
            needOlderData = true
                ...
                if currentPage > 1 {
                    ...
                    if let oldestInPage = pageResponse.results.last,
                       ...
                       oldestInPage.interval_end <= localMax {
                        ...
                        print("Debug - Found \(newRecords.count) new records in page \(currentPage)")
                    ...
                    print("Debug - Storing \(pageResponse.results.count) records from page \(currentPage)")
                    try await storeConsumptionRecords(pageResponse.results)
                    hasMore = pageResponse.next != nil
                } else {
                    // Store first page results
                    print("Debug - Storing \(firstPageResponse.results.count) records from first page")
                ...
                if currentPage == totalPages {
                    ...
                    if let newestInPage = pageResponse.results.first,
                       ...
                       newestInPage.interval_start <= localMin {
                        ...
                        let oldestInPage = pageResponse.results.last?.interval_start ?? newestInPage.interval_start
                        ...
                        print("Debug - Found \(newRecords.count) older records in page \(currentPage)")
                        if !newRecords.isEmpty {
                            try await storeConsumptionRecords(newRecords)
                        }
                        hasMore = false
        ...
        if let minDate = finalData.compactMap({ $0.value(forKey: "interval_start") as? Date }).min(),
        ...
        return try await withCheckedThrowingContinuation { continuation in
            context.perform {
                do {
                    ...
                    print("Debug - Storage update: \(updatedCount) updated, \(insertedCount) inserted")
                    try self.context.save()
                    continuation.resume()
                } catch {
    ...
    private func hasMissingRecords(from startDate: Date, to endDate: Date, localData: [NSManagedObject]) -> Bool {
        ...
        let recordsInRange = localData.filter { record in
            guard let recordStart = record.value(forKey: "interval_start") as? Date,
                  let recordEnd = record.value(forKey: "interval_end") as? Date else {
                return false
            }
